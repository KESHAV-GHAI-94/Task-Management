...
const [editText, setEditText] = useState({});
  ...
  try {
    const res = await axios.post(
      `http://localhost:4000/comment/${id}/comments`,
      {},
      { withCredentials: true }
    );
  ...
  try {
    await axios.post(
      `http://localhost:4000/comment/${id}/addcomment`,
      {
        comment: newComment
      },
      { withCredentials: true }
...
const handleReply = async (commentId) => {
  ...
  try {
    await axios.post(
      `http://localhost:4000/comment/${commentId}/reply`,
      { comment: text },
      { withCredentials: true }
    );
...
const handleDeleteComment = async (commentId) => {
  try {
    await axios.post(
      `http://localhost:4000/comment/${commentId}/delete`,
      {},
      { withCredentials: true }
    );
...
const handleEditComment = async (commentId) => {
  ...
  try {
    await axios.post(
      `http://localhost:4000/comment/${commentId}/update`,
      { comment: text },
      { withCredentials: true }
    );
            ...
            <div className="mt-0">
              ...
              `}
                        ...
                        <div className="flex-1">
                          ...
                          <div
  className={`
    mt-1 px-4 py-3 rounded-lg border text-sm
    ${isMine
...
{comment.replies && comment.replies.length > 0 && (
  <div className="ml-10 mt-3 space-y-2">
    {comment.replies.map((reply) => {
      const isReplyMine = reply.author?.id === currentUser?.id;
      return (
